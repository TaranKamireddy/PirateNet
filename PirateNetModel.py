...
#Data Retrieval
#Originally, we had used the folder of Sonar Images and parsed through them and 
#manually flattened the picture into pixels on a gray scale, but that was taking
#a long time so we decided to use pickle to store the formatted data. This was also
#convient since we need fewer libraries for this code to work and we didn't have to 
...
model = tf.keras.models.Sequential([
  tf.keras.layers.Flatten(input_shape=(507, 1024)), #Matrices dimensions are 507x1024 and this sets the input layer
  tf.keras.layers.Dense(256, activation='relu'),    # Hidden Layer 1 with 256 nodes
  tf.keras.layers.Dropout(0.2),                     #Dropout function is used to break any overfitting of data for the hidden layer
